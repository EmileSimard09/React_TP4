...
export const storageUsernameRealKey = "user_name_real";
...
export const setLocalToken = (authData: IAuthResponse): void => {
  ...
  localStorage.setItem(storageUsernameRealKey, authData.user.username);
...
export const unsetLocalToken = (): void => {
  ...
  localStorage.removeItem(storageUsernameRealKey);
          ...
          if (error.response.data.code === "token_not_valid") {
            ...
            if (refreshToken) {
              ...
              if (tokenParts.exp > now) {
                return CustomAxios.post("auth/token-refresh/", {
                  ...
                  .then((response) => {
                    ...
                    if (error.config) {
                      ...
                      console.log(
                        "Axios error.config is null - auth/token-refresh/"
                      );
                    }
                  })
                  .catch((err) => {
                    console.log(
                      "Axios error handler - auth/token-refresh/",
                      err,
                      err.response
                    );
                  });
